...
export const configs: ConfigEntry[] = [
  ...
  },
  {
    ...
    verification: "The producer logs its effective config with enable.idempotence = true (and acks = all) at startup. To prove the dedup itself, send uniquely tagged test records through an induced retriable error, then consume and confirm each tag appears exactly once.",
  ...
  },
  {
    ...
    verification: "kafka-configs.sh --bootstrap-server <b> --entity-type topics --entity-name <t> --describe shows the override; on a replication-factor-3 topic set to 3, stopping one broker then returns NOT_ENOUGH_REPLICAS for acks=all writes.",
  ...
  },
  {
    ...
    verification: "The gap only shows when a poll() can run before the previous batch's work finishes (e.g. processing is handed to another thread). In that setup, kill the app after a poll but before that work completes: with auto-commit on, the next poll already committed those offsets and the unfinished records are skipped on restart; with it off and a manual commit after the work, they are redelivered.",
