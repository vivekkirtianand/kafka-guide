...
export const configs: ConfigEntry[] = [
  ...
  },
  {
    ...
    verification: "The producer logs its effective config with enable.idempotence = true (and acks = all) at startup. To prove the dedup, the injected fault has to be an ambiguous write — the broker appends but the acknowledgement is lost, forcing a retry of an already-stored batch: with idempotence on the consumer sees each tagged record once, and a control run with it off duplicates the tag.",
  ...
  },
  {
    ...
    verification: "kafka-configs.sh --bootstrap-server <b> --entity-type topics --entity-name <t> --describe shows the override. On a replication-factor-3 topic set to 3, stop one broker and wait until kafka-topics.sh --describe reports Isr with 2 entries — the next acks=all write then fails with NOT_ENOUGH_REPLICAS (an immediate write can still succeed before the leader drops the missing follower from the ISR).",
    rollback: "Restore the value you captured before the change (kafka-configs.sh ... --alter --add-config min.insync.replicas=<prev>). Only use --delete-config if the topic had no override and was inheriting the broker default — deleting it reverts to that default, which is not necessarily 1.",
  ...
  },
  {
    ...
    verification: "Producer batch-size-avg, records-per-request-avg, and compression-rate-avg rise while request rate to the broker falls.",
  ...
  },
  {
    ...
    rollback: "First check the old group still has offsets (kafka-consumer-groups.sh --describe --group <old>) — an inactive group's offsets expire after offsets.retention.minutes. If they survive, restarting under the old id resumes from them (now behind by whatever the new id consumed); if they are gone, the old id falls back to auto.offset.reset.",
  ...
  },
  {
    ...
    verification: "The gap only shows when a poll() runs before the previous batch's work finishes (e.g. processing is handed to another thread). Hand off a batch, then keep polling until kafka-consumer-groups.sh --describe shows the committed offset has moved past it (auto-commit only fires once auto.commit.interval.ms has elapsed) while that work is still unfinished, and kill the app: with auto-commit on the unfinished records are skipped on restart; with it off and a manual commit after the work, they are redelivered.",
