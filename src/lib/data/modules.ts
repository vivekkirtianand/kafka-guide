...
export const modules: Module[] = [
  ...
  },
  {
    ...
    topicNarrative: {
      ...
      "Cooperative assignment":
        "Cooperative assignment (the CooperativeStickyAssignor, the default in modern Kafka) changes what a rebalance costs. Instead of every consumer revoking everything and starting over, the assignor computes the new distribution, and only the partitions that actually need to move are revoked — in a first rebalance round the losing consumers give up just those partitions, and a second round assigns them to their new owners. Consumers keep processing every partition they're not losing straight through the rebalance.\n\nThe practical effect: adding one consumer to a large group moves a handful of partitions and pauses only those, instead of pausing the entire group. The \"sticky\" part means the assignor also tries to keep partitions with their existing owner across rebalances, so a transient membership blip doesn't reshuffle everything. Switching a running group from the eager assignor to the cooperative one requires a specific two-step rolling upgrade — both assignors listed together first, then the old one removed — because the group can't run both protocols at once.",
      "Poison messages and retry strategies":
        "A poison message is a record the consumer can't process successfully no matter how many times it tries — malformed payload, a schema it can't deserialize, a business rule it always violates. The default behavior is the dangerous one: the consumer throws, doesn't commit past the record, and on the next poll gets the same record again. The partition stops advancing, lag grows without bound, and every record behind the poison one is blocked even though nothing is wrong with them.\n\nThe standard fix is to bound in-place retries (a few immediate attempts for genuinely transient failures) and then get the bad record out of the main flow. A dead-letter topic is the common destination: after max attempts, the consumer produces the record — plus metadata about why it failed — to a separate DLT, commits past it on the original partition, and moves on. Head-of-line blocking is gone and the failed records are retained for inspection or manual replay. Non-blocking retry topics (a chain of topics each with an increasing delay) are the more elaborate version, used when failures are often transient-but-slow and you want delayed retries without holding up the main partition. The invariant across all of these: never advance the committed offset past a record until it has either been processed or deliberately routed somewhere durable.",
    },
    activities: [
