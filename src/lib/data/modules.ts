...
export const modules: Module[] = [
    ...
    exercises: [
      {
        ...
        successCriteria: [
          ...
          "You use the full docker exec plus --bootstrap-server form for every command, matching whichever lab you are actually running (localhost:9092 inside kafka-lab-a for Lab A, kafka-1:19092 inside kafka-lab-kafka-1 for Lab B)",
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "On Lab A or Lab B (from Module 2), verify two independent claims about broker and topic behavior — that retention only removes whole closed segments, and that quotas throttle rather than reject — using resources scoped to this exercise so you do not disturb any other lab or module's topics. Use a fresh topic name and a fresh client-id every time you redo this — broker-lab-1 and quota-test-1 the first time, bump the numbers on any later attempt (a fresh client-id is not strictly required to redo the quota part, but keeps things unambiguous if two runs overlap). Every CLI command below needs the full docker exec form for whichever lab you are on: Lab A — docker exec kafka-lab-a /opt/kafka/bin/<tool> --bootstrap-server localhost:9092; Lab B — docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. PART A — retention vs. segment rolling. (1) Create a topic with an aggressively short retention: --create --topic broker-lab-1 --partitions 1 --replication-factor 1 (Lab A) or 3 (Lab B) --config cleanup.policy=delete --config retention.ms=1000. (2) Produce three short records to it with the console producer. (3) Consume them once with --from-beginning --max-messages 3 --timeout-ms 20000 to confirm all three are there. (4) Wait at least 10 seconds — comfortably past the 1000ms retention.ms you set — without producing anything else. (5) Consume again with the exact same command as (3). (6) Explain what you observe: are all three records still there, or gone? Ground the answer in the module's own point about segment granularity — retention.ms only makes a segment eligible for deletion once that segment has rolled (closed), and this partition's tiny, low-traffic active segment is nowhere near the default segment.bytes (1GB) or segment.ms (7 days) it would need to roll on its own. PART B — quotas throttle, they do not reject. (7) Set an aggressively low producer byte-rate quota on a client-id you make up for this: kafka-configs.sh --entity-type clients --entity-name quota-test-1 --alter --add-config producer_byte_rate=500. Confirm it took with the matching --describe. (8) Build one long line — printf 'x%.0s' {1..3000} in bash or zsh — and time how long it takes to produce it to broker-lab-1 with --producer-property client.id=quota-test-1 (wrap the docker exec pipeline in the shell's time builtin). (9) Time the exact same line produced to the same topic again, but with a different --producer-property client.id (anything other than quota-test-1, so no quota applies). (10) Compare the two durations — the quota-test-1 run should take several seconds longer (with a 500 bytes/sec cap on a roughly 3000-byte line, on the order of six seconds, though the exact number depends on your machine), while the unthrottled run returns in well under a second. (11) Confirm both records actually landed — consume the topic and count two records, not one — so you know the slow run was delayed, not rejected. (12) Clean up: kafka-configs.sh --entity-type clients --entity-name quota-test-1 --alter --delete-config producer_byte_rate.",
        successCriteria: [
          "You use a fresh topic name (and, if redoing the quota part in the same sitting, a fresh client-id), bumping the numbers on any later attempt",
          "You use the full docker exec plus --bootstrap-server form for every command, matching whichever lab you are on",
          "You set retention.ms=1000 at topic creation and confirm, after waiting well past that window, that all three records are STILL readable — not deleted",
          "You explain the result in terms of segment rolling, not a broken retention setting: a segment must be closed before retention can act on it, and this partition's active segment never approached segment.bytes or segment.ms, so nothing was ever eligible for deletion",
          ...
          "You confirm both records were actually written — the throttled client was delayed, not rejected, and produced no error at all",
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "This exercise needs Lab B specifically — Lab A has no Grafana, Prometheus, or kafka-exporter, so the dashboard part has nothing to read. Every CLI command below runs as docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. PART A — read the same signal on the dashboard and the CLI. (1) Confirm the whole stack is healthy first: docker compose ps should show all three brokers, kafka-ui, prometheus, and grafana Up. (2) Open http://localhost:3001 (Grafana, anonymous access) -> Dashboards -> \"Kafka lab overview\", and note the current \"brokers reporting\" (3) and \"under-replicated partitions\" (0) panel values. (3) In a terminal, run kafka-topics.sh --describe --under-replicated-partitions and confirm it prints nothing — nothing is under-replicated yet. (4) docker compose stop kafka-2. (5) Within about 30 seconds, re-check Grafana: \"brokers reporting\" should drop to 2 and \"under-replicated partitions\" should climb above 0. (6) Re-run the exact --describe --under-replicated-partitions command from (3) — it should now list every partition that lost kafka-2 from its ISR. (7) Say, in your own words, why the dashboard panel and this CLI command agree — what they are both actually reading. (8) docker compose start kafka-2; wait about 15 seconds; confirm both the dashboard and the CLI command are back to healthy (0 under-replicated, 3 brokers reporting). PART B — tell a flat backlog from a growing one, per partition. (9) Create a fresh topic — observability-lab-1 the first time, bump the number on any later attempt since re-creating an existing topic name fails outright — with 3 partitions and replication factor 3: kafka-topics.sh --create --topic observability-lab-1 --partitions 3 --replication-factor 3. (10) Seed it and fully drain it in one go: from the repo root, cd examples/order-pipeline-java && ./gradlew run --args=\"localhost:29092 20 observability-lab-1\" to send 20 orders, then docker exec kafka-lab-kafka-1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka-1:19092 --topic observability-lab-1 --group lag-demo --from-beginning --timeout-ms 20000 (no --max-messages — let it idle out after reading everything, so its periodic auto-commit has time to fire). (11) --describe --group lag-demo and confirm LAG is 0 on every partition. (12) Seed a second, larger batch — cd examples/order-pipeline-java && ./gradlew run --args=\"localhost:29092 40 observability-lab-1\" — and this time do NOT run any consumer afterward. (13) --describe --group lag-demo again, immediately: LAG is now non-zero, and the per-partition numbers are not identical to each other — say why (the producer's four order templates are not evenly spread across customers, and same-customer records always land on the same partition, so the second batch loads the three partitions unevenly). (14) Wait about 15 seconds, with nothing producing or consuming in between, and --describe --group lag-demo one more time: the LAG should be exactly what it was in (13) — unchanged. Contrast this explicitly with a genuinely growing backlog: flat lag here just means nothing is currently reading it, not that anything is wrong, whereas a slope climbing between two checks is the signal that actually pages someone.",
        successCriteria: [
          ...
          "After stopping kafka-2, both the Grafana panel and the CLI --describe --under-replicated-partitions list agree that partitions are now under-replicated",
          ...
          "You confirm both views return to healthy after restarting kafka-2, rather than assuming it without checking",
          "You use a fresh topic name for Part B, bumping the number on any later attempt",
          ...
          "Your final --describe, taken after waiting with nothing running in between, shows lag UNCHANGED from the previous check — and you correctly describe that as flat, not evidence of a problem, distinct from a rising slope",
        ],
      },
    ],
    activities: [
