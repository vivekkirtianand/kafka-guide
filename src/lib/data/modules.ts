...
export const modules: Module[] = [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "On Lab A or Lab B (from Module 2), verify two of this module's claims about record-size limits for yourself using a topic you create just for this. Use a fresh topic name every time you redo this exercise — producer-lab-1 the first time, bump the number on any later attempt, since kafka-topics.sh --create fails outright with \"Topic already exists\" against a name you have already used. Every CLI command below needs the full docker exec form for whichever lab you are on: Lab A — docker exec kafka-lab-a /opt/kafka/bin/<tool> --bootstrap-server localhost:9092; Lab B — docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. There are THREE separate produce commands below (steps 2, 3, and 4), and every one of them pipes a line into kafka-console-producer.sh's stdin, so every one of them needs docker exec -i (the -i keeps stdin attached — without it the producer receives nothing and that particular step proves nothing, even though it may look like it ran fine); --create, --describe, and kafka-configs.sh do not pipe anything and use plain docker exec, no -i. (1) Create the topic: --create --topic producer-lab-1 --partitions 1 --replication-factor 1 (Lab A) or --replication-factor 3 (Lab B). (2) Trigger the producer's OWN local cap: pipe one long line — printf 'x%.0s' {1..500} in bash or zsh builds a 500-character line — into docker exec -i ... kafka-console-producer.sh --topic producer-lab-1 --producer-property max.request.size=200 --producer-property retries=0. This record is already over the 200-byte limit you just set on the producer itself; nothing about the topic or the broker has changed. (3) Pipe the same 500-character line into docker exec -i ... kafka-console-producer.sh --topic producer-lab-1 again, this time with no producer-property overrides at all (the producer's default max.request.size is far larger), and confirm it succeeds — so you know the failure in (2) came from that override, not from the topic or the record's content. (4) Now trigger the BROKER's own cap instead: leave every producer-property at its default, and first shrink the topic's own limit: kafka-configs.sh --entity-type topics --entity-name producer-lab-1 --alter --add-config max.message.bytes=200. Pipe the same 500-character line into docker exec -i ... kafka-console-producer.sh --topic producer-lab-1 once more — still with no max.request.size override. (5) Explain, from what you configured (not from guessing), why (2) and (4) both print RecordTooLargeException yet are two different checks: (2) is the producer's own max.request.size, enforced against every record before it is ever batched or sent, entirely independent of anything about the topic or the broker; (4) is the broker's per-topic max.message.bytes, which only a record that already passed the producer's own check ever reaches. (6) Clean up the topic override so it does not linger: kafka-configs.sh --entity-type topics --entity-name producer-lab-1 --alter --delete-config max.message.bytes.",
        successCriteria: [
          "You use a fresh, never-before-used topic name (bumping the number on any redo), since re-running --create against an existing topic name fails outright",
          "You use the full docker exec plus --bootstrap-server form for every command, matching whichever lab you are actually running (localhost:9092 inside kafka-lab-a for Lab A, kafka-1:19092 inside kafka-lab-kafka-1 for Lab B), and you attach docker exec -i to ALL THREE piped console-producer commands (steps 2, 3, and 4) — without it on any one of them, that step's line never reaches the producer and looks like a no-op instead of the result it is supposed to prove",
          "In step 2 you set max.request.size as a --producer-property (a client-side override), not a topic or broker config, and get RecordTooLargeException",
          "In step 3 you confirm the very same long record succeeds once max.request.size is left at its default, isolating the producer's own cap as the cause in step 2 — not the topic, not the record's content",
          "In step 4 you set max.message.bytes as a topic config via kafka-configs.sh (not a producer property), and reproduce the same exception with no client-side override in place",
          "You correctly attribute step 2's rejection to the producer's own local check — a record over max.request.size never leaves the process, since it is enforced synchronously inside send() before batching or a network call — and step 4's rejection to the broker's per-topic max.message.bytes, which only ever sees a record the producer's own check already let through",
          "You clean up the max.message.bytes override at the end, rather than leaving the topic's own limit shrunk",
        ],
      },
    ],
    activities: [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "On Lab A or Lab B (from Module 2), verify two independent claims about broker and topic behavior — when retention.ms actually takes effect, and that quotas throttle rather than reject — using a separate topic for each part so neither test disturbs the other or any other lab or module's topics. Use fresh topic names and a fresh client-id every time you redo this — broker-lab-1 and quota-lab-1 for the two topics, quota-test-1 for the client-id, the first time; bump all three numbers together on any later attempt. Every CLI command below needs the full docker exec form for whichever lab you are on: Lab A — docker exec kafka-lab-a /opt/kafka/bin/<tool> --bootstrap-server localhost:9092; Lab B — docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. The console-producer commands below pipe a line into its stdin and need docker exec -i (without -i, stdin never reaches the producer and the piped line is silently lost); --create, --describe, and kafka-configs.sh do not pipe anything and use plain docker exec, no -i. PART A — retention.ms only takes effect on the broker's own schedule, not instantly. (1) Create a topic with an aggressively short retention: --create --topic broker-lab-1 --partitions 1 --replication-factor 1 (Lab A) or 3 (Lab B) --config cleanup.policy=delete --config retention.ms=1000. (2) Produce three short records to it with the console producer (docker exec -i ...). (3) Consume them once with --from-beginning --max-messages 3 --timeout-ms 20000 to confirm all three are there. (4) Now wait — not 10 seconds, but a genuine 6 minutes, doing nothing else on this topic in the meantime. Kafka's own docs are explicit that an expired retention.ms does force a segment roll on its own, independent of segment.bytes and segment.ms, but that only happens when the broker's periodic retention scan runs — log.retention.check.interval.ms, 300000ms (5 minutes) by default, and read-only (no dynamic override, so you cannot shorten it for this exercise) — so a 10-second wait proves nothing either way; 6 minutes comfortably clears one full scan cycle. (5) Consume again with the exact same command as (3). (6) This time the three records should be gone — --max-messages 3 --timeout-ms 20000 now times out with nothing read. Explain why in terms of what actually happened, in order: retention.ms elapsed, so the next retention scan forced the active segment to roll (closed, even though it was nowhere near segment.bytes or segment.ms on its own), and because every record in that now-closed segment was already past retention.ms, the same scan deleted it. Retention was never instantaneous here — it was gated entirely by when that periodic scan happened to run. PART B — quotas throttle, they do not reject. This part uses its own fresh topic (quota-lab-1), untouched by Part A, so the record count below is unambiguous. (7) Create it: --create --topic quota-lab-1 --partitions 1 --replication-factor 1 (Lab A) or 3 (Lab B). (8) Set an aggressively low producer byte-rate quota on a client-id you make up for this: kafka-configs.sh --entity-type clients --entity-name quota-test-1 --alter --add-config producer_byte_rate=500. Confirm it took with the matching --describe. (9) Build one long line — printf 'x%.0s' {1..3000} in bash or zsh — and time how long it takes to produce it to quota-lab-1 with --producer-property client.id=quota-test-1 (wrap the docker exec -i pipeline in the shell's time builtin). (10) Time the exact same line produced to quota-lab-1 again, but with a different --producer-property client.id (anything other than quota-test-1, so no quota applies). (11) Compare the two durations — the quota-test-1 run should take several seconds longer (with a 500 bytes/sec cap on a roughly 3000-byte line, on the order of six seconds, though the exact number depends on your machine), while the unthrottled run returns in well under a second. (12) Confirm both records actually landed: consume quota-lab-1 from the beginning and count exactly two records — since this topic has never held anything else, that count alone confirms the slow run was delayed, not rejected. (13) Clean up: kafka-configs.sh --entity-type clients --entity-name quota-test-1 --alter --delete-config producer_byte_rate.",
        successCriteria: [
          "You use fresh names for both topics and the client-id, bumping all three together on any later attempt",
          "You use the full docker exec plus --bootstrap-server form for every command, matching whichever lab you are on, and attach docker exec -i specifically on every piped console-producer command",
          "You wait a genuine ~6 minutes in Part A, not 10 seconds, and explain why: log.retention.check.interval.ms (default 5 minutes) is a read-only broker setting, so nothing about an expired retention.ms is enforced until that periodic scan next runs",
          "You correctly predict and confirm that the three records ARE eventually gone — not that they survive indefinitely — once that scan has had a full cycle to run",
          "You explain the mechanism precisely: retention.ms itself forces the active segment to roll once it has elapsed (independent of segment.bytes/segment.ms), and the very next retention scan then deletes that now-closed, fully-expired segment — retention only ever takes effect on the broker's own schedule, never instantly",
          "You use a separate, never-touched topic (quota-lab-1) for Part B, so its final record count is unambiguous and not affected by Part A's own topic",
          "You set producer_byte_rate as a CLIENT quota (kafka-configs.sh --entity-type clients --entity-name <your client-id>), not a topic or broker config",
          "Your quota-throttled run takes noticeably longer (several seconds, not milliseconds) than the identical line produced with a different, unthrottled client-id",
          "You confirm quota-lab-1 holds exactly two records — the throttled client was delayed, not rejected, and produced no error at all",
          "You clean up the producer_byte_rate override afterward",
        ],
      },
    ],
    activities: [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "This exercise needs Lab B specifically — Lab A has no Grafana, Prometheus, or kafka-exporter, so the dashboard part has nothing to read. Every docker compose command below runs from the lab directory: cd \"$(git rev-parse --show-toplevel)/local-cluster-lab\" first (skip it if you are already there). Every CLI command below runs as docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. PART A — read the same signal on the dashboard and the CLI. (1) Confirm the whole stack is healthy first: docker compose ps should show all three brokers, kafka-ui, prometheus, and grafana Up. (2) Open http://localhost:3001 (Grafana, anonymous access) -> Dashboards -> \"Kafka lab overview\", and note the current \"brokers reporting\" (3) and \"under-replicated partitions\" (0) panel values. (3) In a terminal, run kafka-topics.sh --describe --under-replicated-partitions and confirm it prints nothing — nothing is under-replicated yet. (4) docker compose stop kafka-2. (5) Poll kafka-topics.sh --describe --under-replicated-partitions every few seconds until it lists every partition that lost kafka-2 from its ISR — do not assume a fixed number of seconds is enough, since ISR shrink detection, Prometheus's scrape interval, and Grafana's own panel refresh interval are three independent clocks. (6) Once the CLI confirms it, switch to Grafana: within its own refresh interval it should now show \"brokers reporting\" at 2 and \"under-replicated partitions\" above 0 — the CLI is your ground truth; the dashboard is the same fact one refresh cycle behind. (7) Say, in your own words, why the dashboard panel and the CLI command agree — what they are both actually reading. (8) docker compose start kafka-2, then poll the same --describe --under-replicated-partitions command until it prints nothing again, rather than assuming any fixed wait is enough; once the CLI confirms it, confirm Grafana has caught up too (0 under-replicated, 3 brokers reporting). PART B — tell a flat backlog from a growing one, per partition, and know what flat does and does not tell you. (9) Create a fresh topic and a fresh consumer group name together — observability-lab-1 and lag-demo-1 the first time, bump both numbers together on any later attempt, since a group name reused across a redo would still carry its previous topic's own committed offsets and --describe would list those old rows alongside the new ones: kafka-topics.sh --create --topic observability-lab-1 --partitions 3 --replication-factor 3. (10) Seed it and fully drain it in one go: from the repo root, cd examples/order-pipeline-java && ./gradlew run --args=\"localhost:29092 20 observability-lab-1\" to send 20 orders, then docker exec kafka-lab-kafka-1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka-1:19092 --topic observability-lab-1 --group lag-demo-1 --from-beginning --timeout-ms 20000 (no --max-messages — let it idle out after reading everything, so its periodic auto-commit has time to fire). (11) --describe --group lag-demo-1 and confirm LAG is 0 on every partition. (12) Seed a second, larger batch — cd examples/order-pipeline-java && ./gradlew run --args=\"localhost:29092 40 observability-lab-1\" — and this time do NOT run any consumer afterward. (13) --describe --group lag-demo-1 again, immediately: LAG is now non-zero, and the per-partition numbers are not identical to each other — say why (the producer's four order templates are not evenly spread across customers, and same-customer records always land on the same partition, so the second batch loads the three partitions unevenly). (14) Wait about 15 seconds, with nothing producing or consuming in between, and --describe --group lag-demo-1 one more time: the LAG should be exactly what it was in (13) — unchanged. Say precisely what that flat reading does and does not tell you here: in this drill it is expected and harmless, because you are the one who stopped consuming — it is not evidence that everything is fine in general. A flat number in production still has to clear your latency SLA and sit well inside retention (this module's own point: slope first, then the absolute value), and a consumer group that has gone quiet when nothing told it to is itself an incident, not something a flat lag reading excuses.",
        successCriteria: [
          "You confirm you are on Lab B, not Lab A, before starting Part A — the dashboard panels do not exist on Lab A at all",
          "You cd into local-cluster-lab (via git rev-parse --show-toplevel) before any docker compose command, rather than running it from wherever you happened to be",
          "You note the healthy baseline (0 under-replicated, 3 brokers reporting, empty --describe --under-replicated-partitions output) before stopping kafka-2",
          "You poll --describe --under-replicated-partitions until it actually confirms the change, both after stopping and after restarting kafka-2, rather than trusting a fixed number of seconds — and only then check whether Grafana has caught up, since it lags the CLI by its own refresh interval",
          "You correctly identify that both views are reading the same underlying fact — brokers missing from partitions' ISRs — through two different tools, not two different problems",
          "You use a fresh topic name AND a fresh consumer group name together for Part B, bumping both numbers on any later attempt — reusing the group name alone would carry stale rows from a previous topic into every later --describe",
          "You fully drain the first batch with a consumer before checking LAG, and confirm it reaches exactly 0 on every partition — not just the group total",
          "You seed the second batch WITHOUT running any consumer afterward, so the non-zero lag you see next is unambiguously caused by unread production, not a race with a consumer that is still catching up",
          "You correctly explain the uneven per-partition lag as key-based partitioning (same customer, same partition), not a bug or something you did wrong",
          "You correctly describe the final unchanged reading as expected in THIS drill, because you deliberately stopped consuming — not as general proof that flat lag is safe; you also state that a flat number still must clear the latency SLA and retention window in production, and that a consumer group gone quiet unexpectedly is itself an incident",
        ],
      },
    ],
    activities: [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "This module has no lab of its own — reproduce and resolve one of its classic incidents yourself, on Lab B (from Module 2), using a topic scoped to this exercise. Use a fresh topic name every time you redo this — hot-partition-lab-1 the first time, bump the number on any later attempt, since re-creating an existing topic name fails outright. Every CLI command below runs as docker exec kafka-lab-kafka-1 /opt/kafka/bin/<tool> --bootstrap-server kafka-1:19092. The two producer commands below pipe generated lines into kafka-console-producer.sh's stdin and need docker exec -i (without -i, stdin never reaches the producer and nothing is actually sent); --create and the consumer command do not pipe anything and use plain docker exec, no -i. (1) Create the topic: --create --topic hot-partition-lab-1 --partitions 3 --replication-factor 3. (2) Produce a deliberately skewed load — one dominant key and two minor ones, simulating a single oversized tenant: in your shell, build and pipe (for i in $(seq 1 27); do echo \"mega-tenant:order-$i\"; done; echo \"tenant-b:order-b1\"; echo \"tenant-c:order-c1\") into docker exec -i ... kafka-console-producer.sh --topic hot-partition-lab-1 --property parse.key=true --property key.separator=: — 29 records total, 27 of them keyed to the same tenant. (3) Consume everything with --from-beginning --property print.partition=true --property print.key=true --max-messages 29 --timeout-ms 20000, and read through the output: every mega-tenant line should carry the same partition number as every other mega-tenant line — one hash, one partition, every time. (4) State the evidence a real on-call engineer would actually see for this: one partition carrying roughly 27 of the topic's 29 records while the other two combined carry only 2 — a hot partition, not a generically slow consumer group. (5) Before trying any fix, explain — per this module's own catalog — why simply raising hot-partition-lab-1's partition count would NOT fix this on its own: hash(key) % partitionCount is still exactly one hash for the string \"mega-tenant\", so its records keep landing on one partition no matter how many partitions the topic has; more partitions only helps when load is spread across many keys, or corrects assignment skew, neither of which is the problem here. (6) Now apply a fix that actually works — key salting — on a second fresh topic (hot-partition-lab-2, bumping alongside hot-partition-lab-1 if you redo this): --create --topic hot-partition-lab-2 --partitions 3 --replication-factor 3, then reproduce the same 27 mega-tenant records, but salt each key with a bucket suffix that cycles 0, 1, 2: (for i in $(seq 1 27); do b=$(( (i - 1) % 3 )); echo \"mega-tenant-$b:order-$i\"; done) piped the same way into docker exec -i ... kafka-console-producer.sh --topic hot-partition-lab-2 --property parse.key=true --property key.separator=:. (7) Consume hot-partition-lab-2 with the same --property print.partition=true --property print.key=true flags and confirm the mega-tenant traffic is no longer piled onto one partition — it is now spread across more than one, because mega-tenant-0, mega-tenant-1, and mega-tenant-2 are three distinct keys that can hash to three different partitions, even though every record still ultimately belongs to the same tenant. (8) Name the cost of this fix, not just its benefit — and be precise about which of two different downstream needs it actually breaks. Salting traded one key for three, so Kafka's same-key-same-partition ordering guarantee — which held for every mega-tenant record on hot-partition-lab-1 — no longer holds across mega-tenant-0/1/2 on hot-partition-lab-2. If all a downstream consumer needs is a combined TOTAL for the tenant (a count, a sum), that is recoverable: read all three buckets and combine by stripping the salt back to the original tenant id, since addition does not care what order its inputs arrive in. But if a downstream consumer needs the tenant's records in their one true ORIGINAL cross-partition order, stripping the salt back off does NOT recover that — Kafka never recorded a single interleaved order across three separate partitions in the first place, so there is nothing to reconstruct it from after the fact. That needs the producer to stamp a source-assigned, per-tenant MONOTONIC sequence number on each record before salting, plus real buffering/reordering logic downstream to sort by it — an ordinary wall-clock timestamp is not good enough on its own, since two records can tie, clocks can skew between producer instances, and more than one producer writing for the same tenant can stamp times inconsistently; a timestamp only works here if the application itself already guarantees it is unique and strictly order-preserving per tenant, which a bare record timestamp is not by default. Short of a real sequence token, the consumer has to accept losing strict ordering for that tenant entirely.",
        successCriteria: [
          "You use fresh topic names for both the unsalted and the salted topic, bumping the numbers together on any later attempt",
          "You use the full docker exec plus --bootstrap-server form for every command, and attach docker exec -i specifically on both piped console-producer commands",
          "You confirm every mega-tenant record lands on the SAME partition on hot-partition-lab-1 — not scattered — and name the mechanism as hash(key) % partitionCount, not coincidence",
          "You state the evidence in terms an on-call engineer would actually check — one partition's share of total records — not just that it feels uneven",
          "You correctly explain why more partitions alone would not fix this: the dominant key's hash does not change, so it keeps landing on one partition regardless of partition count",
          "On hot-partition-lab-2 you salt the SAME 27 logical mega-tenant records with a cycling bucket suffix, producing three distinct keys instead of one",
          "You confirm the salted traffic is no longer concentrated on a single partition, and correctly attribute the improvement to now having three distinct keys to hash, not to the new topic being different in some other way",
          "You separate the two downstream cases correctly: a combined TOTAL for the tenant survives salting fine (stripping the salt and combining is order-independent), but the tenant's one true cross-partition ORDER does not — Kafka never recorded it across three separate partitions, so recovering it needs a source-assigned, per-tenant MONOTONIC sequence token plus real reordering logic, not just stripping the salt back off",
          "You do not accept a bare timestamp as a substitute for that sequence token — you can explain why: ties, clock skew across producer instances, and multiple producers writing the same tenant can all make wall-clock timestamps non-unique or out of order, so a timestamp only works if the application itself separately guarantees it is unique and order-preserving per tenant",
          "You name the tradeoff, not just the benefit: salting mega-tenant into three keys means Kafka's same-key-same-partition ordering no longer holds across that tenant's own records, so anything needing a single true order or a combined total for the tenant must recombine all three buckets itself downstream",
        ],
      },
    ],
    activities: [],
