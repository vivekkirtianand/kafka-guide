...
export const modules: Module[] = [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "Right after finishing Lab C — subject order-events-value on BACKWARD with versions [1,2,3] — test a limit of the lab's own setup that the module's \"not on the hot path, but a dependency\" claim doesn't mention: every console client Lab C uses (both the producer and the long-running consumer from step 3) runs via docker exec INTO the schema-registry container itself, kafka-lab-schema-registry — there's no separate client host. (1) With the step-3 consumer still running, stop the registry with docker compose --profile extras stop schema-registry (not down -v) and see what happens to that consumer's terminal; (2) try any docker exec into kafka-lab-schema-registry — the console tools, even a plain curl — and read the exact error; (3) restart it with docker compose --profile extras start schema-registry, then run curl http://localhost:8081/subjects/order-events-value/versions and confirm it still reports [1,2,3]; (4) start a brand-new console consumer on order-events --from-beginning and confirm it decodes all four orders, fetching each of the three schema ids on its first encounter with it; (5) write down, specifically, why this lab's own tooling can't actually test the \"a warm client survives a registry outage\" claim, and what you would need in order to test it for real.",
        successCriteria: [
          ...
          "You bring it back with start (or up -d), not down -v, and confirm /subjects/order-events-value/versions still reports [1,2,3] — nothing was deleted, only paused",
          "You confirm a brand-new from-beginning consumer decodes all four orders once the registry is healthy again, fetching schema ids 1 through 3 as it goes",
