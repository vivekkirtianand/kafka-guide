...
export const modules: Module[] = [
  ...
  },
  {
    ...
    topicNarrative: {
      ...
      "Improve batching (batch.size, linger.ms)":
        "Every produce request is per-partition network and disk I/O on the broker, so sending one record per request wastes most of that overhead on framing rather than payload. Batching amortizes it: the producer accumulates records destined for the same partition into a single batch and sends it as one request.\n\nlinger.ms is how long the producer will wait, after the first record in a batch arrives, before sending it anyway even if the batch isn't full — trading a small amount of added latency for a much fuller, more efficient batch. batch.size is the other trigger: if enough records arrive quickly enough to fill it before linger.ms elapses, the batch sends immediately regardless of the timer. Kafka 4.0 raised the default linger.ms from 0 to 5, on the reasoning that 5ms of added latency is negligible for nearly every workload and the batching win is not.",
      "Control memory and backpressure (buffer.memory, max.block.ms)":
      ...
      "Use transactions (transactional.id, transaction timeouts)":
        "Transactions extend idempotence (which only guarantees exactly-once writes to a single partition) to atomic writes across multiple partitions and topics: either every record in the transaction becomes visible to read_committed consumers, or none of them do. Setting transactional.id turns this on for a producer and also fences out any older producer instance still running with the same ID — the classic \"zombie\" scenario after a restart — which is why transactions require enable.idempotence=true and acks=all underneath.\n\ntransaction.timeout.ms bounds the other failure mode: a transaction that's been started (beginTransaction()) but never committed or aborted, which would otherwise block read_committed consumers from reading past it indefinitely. Once that timeout elapses, the coordinator aborts the transaction on the producer's behalf so consumers aren't left waiting on a producer that may never come back.",
    },
    activities: [
