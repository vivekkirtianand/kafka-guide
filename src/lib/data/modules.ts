...
export const modules: Module[] = [
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          'Verify two of this module\'s claims for yourself against Lab A or Lab B\'s orders topic. Use fresh group names you haven\'t used before — reporting-1, fulfilment-1, commit-timing-1 the first time; bump the number if you ever redo this exercise, since a group that already has a committed offset from a previous attempt ignores --from-beginning and just resumes where it left off. There is no host-installed Kafka CLI — every command below needs docker exec (Lab A: kafka-lab-a, --bootstrap-server localhost:9092; Lab B: kafka-lab-kafka-1, --bootstrap-server kafka-1:19092). (1) Confirm two independent consumer groups really do fan out, not split: run the order-pipeline-java ConsumerApp with group reporting-1 AND the skip policy against orders (./gradlew runConsumer --args="localhost:PORT reporting-1 skip") — skip, not the default propagate, because orders already carries plain-text records from Lab A/B\'s own steps ("first", "west:A", and the like) that aren\'t valid OrderEvent JSON and would crash a propagate-policy consumer on the very first one; let it catch up, then Ctrl-C it; run it again with group fulfilment-1 (same skip policy) against the same topic and confirm its log processes every one of the same records too — not a subset — even though reporting-1 already read them all; check both with kafka-consumer-groups.sh --describe and confirm each group reaches LAG 0 independently, with its own CURRENT-OFFSET matching its own LOG-END-OFFSET. (2) Verify a committed offset lags the read position until the commit interval elapses: start the plain console consumer against orders with a named group, --from-beginning (Kafka 4.0\'s console consumer defaults to auto.offset.reset=latest without it, so a brand-new group would see nothing), and a generous, explicit interval (docker exec ... kafka-console-consumer.sh --group commit-timing-1 --from-beginning --consumer-property auto.commit.interval.ms=60000 --bootstrap-server ...), no --max-messages, and leave it running; once its screen has caught up and gone quiet, --describe group commit-timing-1 in a second terminal and note CURRENT-OFFSET; wait a full 60+ seconds with the consumer still running (don\'t touch it), then --describe again; only then Ctrl-C it. (3) Say, in your own words, why the two --describe checks in (2) gave different numbers even though the consumer had already printed every record on screen well before the first check.',
        successCriteria: [
          "You use the skip poison policy, not the default propagate, for both reporting-1 and fulfilment-1, and explain why: orders already carries plain-text records from Lab A/B that aren't valid JSON and would crash a propagate-policy consumer on the first one",
          "You confirm fulfilment-1's log processes every record reporting-1 already consumed — a different group.id gets its own full copy, not whatever's left over",
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          "Prove you understand Lab D's Connect REST API pattern, not just its exact commands, by building a second, completely independent pipeline through it. Reuse the same worker (docker compose --profile extras up -d kafka-connect if you already tore Lab D's down): (1) inside the Connect container, create the file your source connector will read, with some starting content — same as Lab D's own make-source-file step, and just as necessary: FileStreamSourceTask opens this file at connector startup, so if it doesn't exist yet the task fails immediately instead of waiting for you to append to it later; (2) only now write your own PUT config for a new file source connector — pick a name other than file-source, a file other than /tmp/connect-source.txt (the one you just created), and a topic other than connect-file-topic; (3) confirm it reaches status RUNNING; (4) write your own sink connector config, again with a name and file distinct from Lab D's, consuming your new topic; (5) append a line to your source file and, after a short wait, confirm it flows through to your sink file — the same tail-then-flush behavior Lab D's own append-tail step showed, on infrastructure you configured yourself; (6) GET /connectors and confirm your two connectors are listed under the exact names you chose; (7) delete both by name and confirm GET /connectors no longer lists them — and, if Lab D's own file-source/file-sink (or anything else) was still running the whole time, confirm deleting yours left them completely untouched.",
        successCriteria: [
