...
export const modules: Module[] = [
    ...
    exercises: [
      {
        ...
        successCriteria: [
          ...
          "You find the same group's per-partition current-offset, log-end-offset, and lag in both kafka-consumer-groups.sh --describe and the UI's consumer detail view, and the numbers agree",
  ...
  },
  {
    ...
    exercises: [
      {
        prompt:
          'The consumer-groups walkthrough lesson describes running several ConsumerApp instances in one group, but only ever shows the command for one. Go do it for real, against Lab A or Lab B\'s orders topic — it always has exactly 3 partitions, which this exercise depends on: (1) in three separate terminals, start three ConsumerApp instances all in group team-a (./gradlew runConsumer --args="localhost:PORT team-a"), and from the rebalance-listener log lines, write down exactly which partitions each instance ends up owning; (2) start a fourth team-a instance against the same topic and check what its own log shows about its assignment; (3) stop the three original instances and restart them the same way but with SLOW_MS=300 set, then run the producer with ./gradlew run --args="localhost:PORT 60" — 60 orders is enough backlog that the three are still working through it several seconds later; while they are still processing, Ctrl-C one of the three (not the idle fourth) and read the survivors\' logs for the revoke-then-reassign; (4) once everything goes quiet, run kafka-consumer-groups.sh --describe --group team-a and confirm every partition\'s LAG has reached 0, including the one that changed hands; (5) say what happened to the records still queued on that partition the moment you killed its owner — lost, delayed, or something else, and why.',
        successCriteria: [
          ...
          "After killing one instance, you show the survivors' revoke-then-reassign log lines",
          "You confirm via kafka-consumer-groups.sh --describe --group team-a that every partition's LAG reaches 0, including the reassigned one — not by eyeballing which customer keys came through, which can land on only some partitions and can't prove every one was exercised",
    ...
    exercises: [
      {
        ...
        successCriteria: [
          "You find that the produce in step 1 fails even though the schema text is identical to one already registered — the serializer caches a schema-to-id mapping only after its first successful lookup in a given process, and this console-producer invocation is a brand-new process starting from an empty cache",
