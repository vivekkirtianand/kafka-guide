    ...
    exercises: [
      {
        ...
        successCriteria: [
          ...
          "You separate the two downstream cases correctly: a combined TOTAL for the tenant survives salting fine (stripping the salt and combining is order-independent), but the tenant's one true cross-partition ORDER does not — Kafka never recorded it across three separate partitions, so recovering it needs an explicit sequence field plus real reordering logic, not just stripping the salt back off",
