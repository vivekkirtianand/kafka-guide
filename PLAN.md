...
| 2 (P2) | `--if-not-exists` silently reuses a stale `order-events` topic — a rerun folds old records and schema versions in, breaking the `[1,2]` / `[1,2,3]` checks. | `create-topic` drops `--if-not-exists` and gains a `TopicExistsException` `commonError` pointing at `down -v`. `verify` became a clean-state check (refined further in round 3). Verified against a live stack: a second `--create` errors `TopicExistsException`. |

**Review findings addressed (round 3)**

| # | Finding | Fix |
|---|---|---|
| 1 (P2) | The round-2 clean-state check was imprecise: `curl /subjects` flagged *unrelated* subjects as stale, `grep order-events` also matched `order-events-archive`, and `\| grep … \|\| echo "(no topic)"` turned a broken `docker exec` into a false "clean". | `verify` now runs `curl …/subjects/order-events-value/versions` (that one subject only) `&&` `kafka-topics.sh --describe --topic order-events` (exact name, no `grep`, no `\|\| echo`). Clean = `40401 Subject … not found` + `Topic 'Optional[order-events]' does not exist`; a version array or a real topic = stale; a connection error / `No such container` reads distinctly and the note says that is "not the same as clean". Verified live: `grep order-events` matched both `order-events` and `order-events-archive`; `--describe --topic order-events` and `/subjects/order-events-value/versions` are exact. |
...
## Phase 8 — version & deployment awareness

Makes the version selector tell the truth about the Kafka release landscape, and reconciles
the guide's content with it. Three PRs; 8c (cheap renames) folded into 8a.

**AskUserQuestion decisions:** the lab Docker image **stays at `apache/kafka:4.0.2`** — it is
the latest 4.0.x patch, clears CVE-2026-35554, and matches the 4.0-reviewed content; bumping
it would mean re-verifying all five labs against a fresh broker for no learner-visible gain.
PR split is **8a plumbing, then 8b content**.
...
- **`src/lib/types.ts`**
  - `KAFKA_VERSIONS` → `["4.3", "4.2", "4.1", "4.0", "3.9"]` (newest-first). Dropped 3.7 and
    3.5 — both 2+ years past EOL and only noise on the selector. 3.9 kept: the last 3.x line,
...
- **`src/components/TopBar.tsx`** — version `<option>`s show `· archived`; when an archived
  version is selected, a wrap-friendly note line ("Kafka X is end of life — latest patch …,
  no further releases. Apache supports 4.3, 4.2, 4.1.").
- **`src/components/ConfigExplorer.tsx`** — an archived-version banner above the managed
  banner ("defaults below are for that line; Apache still ships fixes for …").
- **`src/app/page.tsx`** — Configuration explorer card: badge "version + deployment aware" →
...
## Phase 10a — per-lesson knowledge checks

Every content module gets a self-scored multiple-choice check (Module 0's was written in
Phase 2c). The `KnowledgeCheck` type and its generic render in `[slug]/page.tsx` already
exist, so 10a is pure content authoring.

**AskUserQuestion decisions:** **4 PRs by theme** — 10a-1 core concepts (Modules 1, 4),
10a-2 hands-on (2, 3, 5), 10a-3 groups & pipelines (7, 8), 10a-4 reference (6, 9, 10, 11);
**6–8 questions per module**, scaled to module size. Module 12 (capstone) is graded by its
rubric and gets no check.

### PR 10a-1 — core concepts (Modules 1 and 4)

- **`src/lib/data/modules.ts`** — `knowledgeChecks` on `mental-model` (7 Qs: what stamps an
  offset and what it means, reading doesn't consume, retention is age/size not consumption,
  RF 3 needs ≥3 brokers, same key → same partition, read position vs committed offset,
  6 members / 4 partitions → 2 idle) and on `keys-ordering-and-delivery` (8 Qs: default
  partitioner hashes the key, null key → sticky batch-wise spread, a key is not a primary
  key, ordering is per-partition, idempotence holds order via per-partition sequence numbers,
  acks=all + `min.insync.replicas=2` for one-broker durability, commit-before-process is
  at-most-once, idempotent producer ≠ end-to-end EOS). All 4-option, grounded in each
  module's existing `topicDetail`.
- **`src/lib/data/modules.test.ts`** — new `CHECKED_MODULES` list (grows per 10a PR): each
  named module has ≥5 questions, exactly 4 distinct options, a "?"-terminated question, and a
  substantive explanation. Suite 456 → 457.
...
- **`src/lib/data/modules.ts`** — `knowledgeChecks` on:
  - `local-cluster-lab` (7 Qs): what RF-3 / min-ISR-2 buys, the KRaft controller's job,
    why auto-topic-creation is off, host vs in-container listeners, keyed-record partition
    placement in the console tools, offset reset = replay (topic unchanged), stop a broker →
    new leader from the ISR.
  - `build-a-producer-and-consumer` (7 Qs): `send()` is async — confirm via the Future,
    keyed by customerId for ordering, poll→process→commit is at-least-once, why
    `enable.auto.commit=false`, `wakeup()` for a clean group leave, a rethrown parse error
    stalls the partition, skip vs dead-letter (skip discards silently).
  - `schemas-and-data-contracts` (8 Qs): the broker never validates shape, the contract
    exists with or without a registry, JSON's real costs, the wire format carries a 4-byte
    schema id not the schema, a warm consumer survives a registry outage, BACKWARD ⇒
    consumers first, non-transitive BACKWARD gap on replay, `SerializationException` comes
    out of `poll()`.
- **`src/lib/data/modules.test.ts`** — `CHECKED_MODULES` extended to the three new slugs.
